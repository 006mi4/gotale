    ...
    def wrapped(*args, **kwargs):
        try:
            if not current_user.is_authenticated:
                logger.warning(f"[WebSocket] Authentication failed for event")
...
def register_socketio_events(socketio):
    ...
    def handle_join_console(data):
        ...
        try:
            ...
            if auth_status['auth_pending'] and auth_status['auth_url']:
                logger.info(f"[Console] Sending pending auth_required for server {server_id}")
                emit('auth_required', {
                    'server_id': server_id,
            ...
            logger.info(f"User {current_user.username} joined console for server {server_id}")

        except Exception as e:
            logger.error(f"Error joining console: {e}")
    ...
    def handle_join_gotale(data):
        ...
        try:
            ...
            logger.info(f"[GoTaleBridge] Client joined GoTale room for server {server_id}")
            emit('gotale_status', {
                'server_id': server_id,
                'connected': gotale_bridge.get_status(server_id)
            })
        except Exception as exc:
            logger.error(f"Error joining GoTale room: {exc}")
    ...
    def handle_leave_console(data):
        ...
        try:
            ...
            logger.info(f"User {current_user.username} left console for server {server_id}")

        except Exception as e:
            logger.error(f"Error leaving console: {e}")
    ...
    def handle_console_command(data):
        ...
        try:
            ...
            if not server_id or not command:
                logger.warning(f"[Console] Error: Missing server_id or command")
            ...
            if not server:
                logger.warning(f"[Console] Error: Server {server_id} not found in database")
                emit('error', {'message': 'Server not found'})
                return

            # Check if server is running
            if not server_manager.is_server_running(server_id):
                logger.warning(f"[Console] Error: Server {server_id} is not running")
                emit('error', {'message': 'Server is not running'})
                return

            # Send command
            logger.debug(f"[Console] Sending command to server process...")
            success = server_manager.send_command(server_id, command)

            if not success:
                logger.error(f"[Console] Error: Failed to send command to server process")
                emit('error', {'message': 'Failed to send command'})
                return

            logger.debug(f"[Console] Command sent successfully")
            ...
            logger.error(f"[Console] Error sending console command: {e}", exc_info=True)
            emit('error', {'message': 'Failed to send command'})
    ...
    def handle_connect():
        ...
        if current_user.is_authenticated:
            ...
            logger.debug("Unauthenticated user connected via WebSocket")
